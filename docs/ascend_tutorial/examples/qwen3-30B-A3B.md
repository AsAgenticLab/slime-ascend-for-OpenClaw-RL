# QWEN-3-30B-A3B 部署示例
训练脚本位于[run-qwen3-30B-A3B-npu.sh](../../../scripts/ascend_script/run-qwen3-30B-A3B-npu.sh)
...
```bash
# 下载模型权重 (Qwen3-30B-A3B)
hf download Qwen/Qwen3-30B-A3B --local-dir /path/to/Qwen3-30B-A3B
...
首先，加载目标模型的配置文件。`slime/scripts/models` 目录下包含了支持模型的配置文件。需要 `source` 对应模型的脚本，将配置参数加载到当前环境中。此处我们以 Qwen3-30B-A3B 模型为例子。
...
```bash
cd /path/to/slime-ascend
bash scripts/ascend_script/run-qwen3-30B-A3B-npu.sh
```
